...
pub fn path_from_ordered_root_with_block_tree(
    block_id: HashValue,
    block_tree: Arc<RwLock<BlockTree>>,
) -> Option<Vec<Arc<ExecutedBlock>>> {
    block_tree.read().path_from_ordered_root(block_id)
}
...
pub fn recover_executor_block_tree_and_retry_execute_with_block_tree_and_state_computer(
    ...
    let blocks_to_reexecute = path_from_ordered_root_with_block_tree(parent_block_id, block_tree)
